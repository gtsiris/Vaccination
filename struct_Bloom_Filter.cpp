...
bloomFilter::bloomFilter(const unsigned int& size) : size(size) {
	if (size % sizeof(int) == 0)
		bitArray = new int[size / sizeof(int)];
	else
		bitArray = new int[size / sizeof(int) + 1];
}
...
bool bloomFilter::Search(const string& key) const {
	...
	for (unsigned int i = 0; i < K; i++) {
		unsigned int pos = (hash_i(buffer, i) % (size * 8)) / (sizeof(int) * 8);  /* Position of the desired integer */
		unsigned int index = (hash_i(buffer, i) % (size * 8)) % (sizeof(int) * 8);  /* Index inside the desired integer */
...
void bloomFilter::Insert(const string& key) {
	unsigned char *buffer = new unsigned char[strlen(key.c_str())];
	strcpy((char *)buffer, key.c_str());
	for (unsigned int i = 0; i < K; i++) {
		unsigned int pos = (hash_i(buffer, i) % (size * 8)) / (sizeof(int) * 8);
		unsigned int index = (hash_i(buffer, i) % (size * 8)) % (sizeof(int) * 8);
		SetBit(pos, index);
	}
}
